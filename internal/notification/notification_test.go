...
`
	...
	t.Run("trusted XDG base expands shell", func(t *testing.T) {
		...
		if !cfg.AllowShellForNotificationTemplate() {
			t.Fatalf("AllowShellForNotificationTemplate() = false, want true for trusted base config")
		}
		...
		if !strings.Contains(notification, "trusted xdg-notification-template") {
			t.Fatalf("notification template did not execute trusted shell fragment: %q", notification)
		}
		if strings.Contains(notification, "$(printf xdg-notification-template)") {
			t.Fatalf("notification template kept literal trusted shell fragment: %q", notification)
		}
	})
