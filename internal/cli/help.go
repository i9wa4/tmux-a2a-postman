...
func runHelp(stdout, stderr io.Writer, args []string) error {
	...
	if len(args) == 0 {
		...
		fmt.Fprintln(stdout, "  status                     Show current runtime status (--json for canonical payload)")
		...
		fmt.Fprintln(stdout, "  status --json                           Inspect runtime status as JSON")
	...
	case "config":
		...
		fmt.Fprintln(stdout, "  get-health, get-health-oneline, status, and the default TUI read the same canonical health contract.")
		fmt.Fprintln(stdout, "")
		fmt.Fprintln(stdout, "Quick reading guide:")
		fmt.Fprintln(stdout, "  visible_state in status JSON answers what the node looks like now")
	...
	case "commands":
		...
		fmt.Fprintln(stdout, "  Show current runtime status.")
		...
		fmt.Fprintln(stdout, "  Flags:")
		fmt.Fprintln(stdout, "    --json               Output canonical all-session status JSON")
		fmt.Fprintln(stdout, "    --session <name>     tmux session name (optional, auto-detected)")
		fmt.Fprintln(stdout, "    --config <path>      Config file path (optional)")
		fmt.Fprintln(stdout, "")
