...
func TestProjectTaskRunState_SeparatesExplicitReviewAndAcceptanceConventions(t *testing.T) {
	...
	for index, item := range []struct{ id, run, body string }{
		...
		{"m3.md", "run-a", "ACCEPTED\nowner decision"},
		{"m4.md", "run-b", "ACCEPTED\nother run"},
	} {
	...
	projected, ok, err := ProjectTaskRunState(sessionDir, "review")
	if err != nil || !ok || len(projected.Tasks) != 2 {
		t.Fatalf("ProjectTaskRunState = %#v ok=%v err=%v", projected, ok, err)
	}
	if projected.Tasks[0].RunID != "run-a" || projected.Tasks[0].ReviewApprovalMessageID != "m2.md" || projected.Tasks[0].AcceptanceMessageID != "m3.md" {
		t.Fatalf("run-a projection = %#v", projected.Tasks[0])
	}
	if projected.Tasks[1].RunID != "run-b" || projected.Tasks[1].ReviewApprovalMessageID != "" || projected.Tasks[1].AcceptanceMessageID != "m4.md" {
