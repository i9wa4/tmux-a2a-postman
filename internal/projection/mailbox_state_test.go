...
func TestProjectMailboxState_ReplaysUnreadCountsForCurrentGeneration(t *testing.T) {
	...
	if _, err := writer.AppendEvent(MailboxProjectionReadEventType, journal.VisibilityOperatorVisible, map[string]string{
		"to": "worker",
...
func TestProjectMailboxState_AcceptsLegacyMailboxProjectionEvents(t *testing.T) {
	sessionDir := t.TempDir()
	...
	if _, err := writer.AppendEvent(legacyCompatibilityMailboxDeliveredEventType, journal.Visibility("compatibility_mailbox"), map[string]string{
		"to": "worker",
	}, now.Add(time.Second)); err != nil {
		t.Fatalf("AppendEvent(legacy delivery): %v", err)
	}
...
func TestProjectMailboxState_ReplaysAcrossLeaseResume(t *testing.T) {
	...
	if _, err := resumedWriter.AppendEvent(MailboxProjectionDeliveredEventType, journal.VisibilityMailboxProjection, map[string]string{
		"to": "critic",
	}, now.Add(3*time.Second)); err != nil {
		t.Fatalf("AppendEvent(second delivery): %v", err)
	}
	if _, err := resumedWriter.AppendEvent(MailboxProjectionReadEventType, journal.VisibilityOperatorVisible, map[string]string{
		"to": "worker",
