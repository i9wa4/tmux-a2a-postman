...
| Daemon-submit requests     | Bounded workers, per-operation active key | watcher create/rename, scan recovery       | active submit key map in event loop        | request claim/read, post write, inbox pop                  | worker cap, per-key filesystem queue      | send and unrelated inbox pops may run together |
