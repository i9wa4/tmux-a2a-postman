...
| Daemon-submit requests     | Bounded workers, per-session active guard | watcher create/rename, scan recovery       | active submit session map in event loop    | request claim/read, post write, inbox pop, projection sync | worker cap, per-session filesystem queue  | worker result returns to event loop            |
...
| Post reconciliation        | Serialized reservation                    | post watcher event, submit result, backlog | active post events, delivery route state   | projection sync, node discovery                            | same-route retry timer                    | pane delivery starts after reservation         |
